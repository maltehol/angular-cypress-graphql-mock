...
interface Window {
  XMLHttpRequest: XMLHttpRequest & IPrototype;
}
...
declare namespace Cypress {
  ...
  interface Chainable {
    ...
    addGraphQLMockMap(queryToMockFnMap: {
      [query: string]: MockFn<unknown>;
    }): void;
...
declare type MockFn<T> = (
  graphQLRequest?: GraphQLRequest<T>
) => string | object;
// read the config
const graphQLEndpoint = Cypress.config("graphQLEndpoint")
  ? Cypress.config("graphQLEndpoint")
  : "/graphql";
const graphQLMethod = Cypress.config("graphQLMethod")
  ? Cypress.config("graphQLMethod")
  : "POST";
...
*/
const graphQLParseRegEx = Cypress.config("graphQLParseRegEx")
  ? Cypress.config("graphQLParseRegEx")
  : /{\s*(?<query>[\w\-]+)(?:\((?<parameter>[^\)]+)\))?(?<body>(?:.*\s)*)}/;
...
Cypress.Commands.add(
  "addGraphQLMockMap",
  (queryToMockFnMap: { [query: string]: MockFn<unknown> }) => {
...
Cypress.Commands.add(
  "addGraphQLMock",
  (query: string, mockFn: MockFn<unknown>) => {
    localStorage.setItem(`${LS_PREFIX}${query}`, mockFn.toString());
  }
);
    ...
    win.XMLHttpRequest.prototype.open = function (method: string, url: string) {
      ...
      this.send = (x: string) => {
        try {
          ...
          console.info(
            "Could not parse graphQL request. We ask the Server instead."
          );
          send.call(this, x);
        }
      };
